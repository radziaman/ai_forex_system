...
class ActorNetwork(nn.Module):
    ...
    def __init__(self, input_dim: int, n_actions: int = 5):
        ...
        self.feature_extractor = nn.Sequential(
            nn.Linear(input_dim, 1024), nn.LayerNorm(1024), nn.Tanh(), nn.Dropout(0.1),
            nn.Linear(1024, 512), nn.LayerNorm(512), nn.Tanh(), nn.Dropout(0.1),
            nn.Linear(512, 256), nn.LayerNorm(256), nn.Tanh(), nn.Dropout(0.1),
            nn.Linear(256, 128), nn.LayerNorm(128), nn.Tanh(), nn.Dropout(0.1),
        )
...
class PPOAgent:
    ...
    def __init__(self, state_dim: int, n_actions: int = 5, lr: float = 3e-4,
                 gamma: float = 0.99, gae_lambda: float = 0.95,
                 clip_range: float = 0.2, ent_coef: float = 0.01,
                 vf_coef: float = 0.5, max_grad_norm: float = 0.5,
                 device: str = "auto"):
        self.device = torch.device("cuda" if torch.cuda.is_available() and device == "auto" else "cpu") if device == "auto" else torch.device(device)
        self.gamma = gamma
        ...
        self.sl_vals = []; self.tp_vals = []; self.size_vals = []

        logger.info(f"PPOAgent initialized | Device: {self.device} | State dim: {state_dim}")
    ...
    def select_action(self, state: np.ndarray) -> Tuple[int, float, float, float, dict]:
        ...
        return action, sl_raw.item(), tp_raw.item(), size_raw.item(), {
    ...
    def train(self, next_value: float = 0.0, n_epochs: int = 10, batch_size: int = 64) -> dict:
        ...
        advantages, returns = self._compute_gae(rewards, dones, values_t.cpu().numpy(), next_value)
        ...
        for _ in range(n_epochs):
            ...
            for start in range(0, len(states_t), batch_size):
                ...
                s2 = torch.clamp(ratio, 1-self.clip_range, 1+self.clip_range) * advantages_t[b_idx]
    ...
    def _compute_gae(self, rewards, dones, values, next_val):
        ...
        for t in reversed(range(len(rewards))):
            next_v = next_val if t == len(rewards)-1 else values[t+1] if not dones[t] else 0.0
    ...
    def save(self, path: str):
        torch.save({'actor': self.actor.state_dict(), 'optimizer': self.optimizer.state_dict()}, path)
...
class TradingEnvironment:
    ...
    def __init__(self, state_dim: int, initial_balance: float = 100000.0,
                 spread_pips: float = 0.5, commission_per_lot: float = 7.0):
    ...
    def step(self, action: int, sl_mult: float = 2.0, tp_mult: float = 4.0,
             size: float = 0.01) -> Tuple[np.ndarray, float, bool, dict]:
        reward = 0.0
        done = False
        info = {'balance': self.balance, 'equity': self.equity, 'open_positions': len(self.open_positions)}

        if action == 1:  # BUY
            self._open_pos("BUY", self.current_price + self.spread_pips*0.0001, sl_mult, tp_mult, size)
        elif action == 2:  # SELL
            self._open_pos("SELL", self.current_price - self.spread_pips*0.0001, sl_mult, tp_mult, size)
    ...
    def _open_pos(self, direction, entry_price, sl_m, tp_m, size):
        sl = entry_price - sl_m*0.001 if direction == "BUY" else entry_price + sl_m*0.001
        tp = entry_price + tp_m*0.001 if direction == "BUY" else entry_price - tp_m*0.001
        margin_req = entry_price * size * 100000 * 0.01
        if self.margin + margin_req > self.balance * 0.8:
            return
        self.open_positions.append({
            'id': self.total_trades+1, 'direction': direction, 'entry': entry_price,
            'sl': sl, 'tp': tp, 'size': size, 'pnl': 0.0
        })
    ...
    def _close_pos(self, pos):
        ...
        pnl = (exit_price - pos['entry']) * pos['size'] * 100000 if pos['direction'] == "BUY" else (pos['entry'] - exit_price) * pos['size'] * 100000
        pnl -= self.commission * pos['size']
        self.balance += pnl
        self.margin -= pos['entry'] * pos['size'] * 100000 * 0.01
        self.trade_history.append({**pos, 'exit': exit_price, 'pnl': pnl, 'win': pnl > 0})
        self.total_pnl += pnl
    ...
    def _calc_reward(self) -> float:
        ...
        max_bal = max([self.initial_balance] + [self.initial_balance + t['pnl'] for t in self.trade_history])
        ...
        return profit * 2.0 + sharpe * 1.5 + dd_pen * 3.0 + trans_cost * 1.0 + win_bonus * 2.0 + margin_pen * 2.0 + consec_pen * 1.5
