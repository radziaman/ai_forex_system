"""
Fixed cTrader Client - Uses CORRECT EndPoints from documentation
Properly implements: https://help.ctrader.com/open-api/account-authentication/
"""
import json
import time
import logging
...
# PPROTOBUF_DEMO_HOST = "demo.ctraderapi.com"  <- CORRECT!
# PPROTOBUF_LIVE_HOST = "live.ctraderapi.com" <- CORRECT!
# Port: 5035
...
class FixedCtraderClient:
    """
    Fixed cTrader client using CORRECT EndPoints.
    Following: https://help.ctrader.com/open-api/account-authentication/
    
    Authentication Flow:
    1. Connect to demo.ctraderapi.com:5035 (or live.ctraderapi.com:5035)
    ...
    5. Send ProtoOAGetAccountInfoReq to get REAL account data
    """
    ...
    def __init__(self, demo: bool = True):
        ...
        if demo:
            self.host = EndPoints.PROTOBUF_DEMO_HOST  # demo.ctraderapi.com
        else:
            self.host = EndPoints.PROTOBUF_LIVE_HOST  # live.ctraderapi.com
        ...
        self._spot_data: Dict[str, SpotData] = {}
        self._ctid_accounts: List[int] = []
        
        logger.info(f"FixedCtraderClient initialized")
        logger.info(f"Host: {self.host}:{self.port}")
        logger.info(f"Account ID: {ACCOUNT_ID} | Demo: {demo}")
    
    def start(self) -> bool:
        """Start and authenticate with cTrader."""
        logger.info("=" * 70)
        logger.info("STARTING FIXED CTRADER CLIENT (CORRECT ENDPOINTS)")
        logger.info("=" * 70)
        logger.info(f"Host: {self.host}:{self.port}")
        ...
        try:
            ...
            logger.error("Make sure ctrader-open-api is installed: pip install ctrader-open-api")
            return False
    ...
    def _authenticate_application(self):
        ...
        logger.info("→ Sent ProtoOAApplicationAuthReq")
        logger.info(f"   Client ID: {APP_CLIENT_ID}")
        logger.info(f"   Client Secret: [hidden]")
        ...
        try:
            ...
            if hasattr(payload, 'ctidTraderAccount'):
                ...
                for acc_id in self._ctid_accounts:
                    logger.info(f"   ctidTraderAccountId: {acc_id}")
                
                # Use the account ID from config or first available
    ...
    def _authenticate_account(self, ctid_trader_account_id: int):
        ...
        msg = Protobuf.get("ProtoOAAccountAuthReq",
                         ...
                         accessToken=ACCESS_TOKEN)
        self._client.send(msg)
        logger.info("→ Sent ProtoOAAccountAuthReq")
...
if __name__ == "__main__":
    print("=" * 70)
    print("AI FOREX TRADING SYSTEM v3.0 - FIXED CTRADER CONNECTION")
    print("Using CORRECT EndPoints: demo.ctraderapi.com:5035")
    print("=" * 70)
    print()
