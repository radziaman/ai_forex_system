"""
FIXED cTrader Client - Patches Twisted SSL context
Makes it work on macOS by using permissive SSL.
...
class PatchedCtraderClient:
    """
    cTrader client with PATCHED SSL context.
    Monkey-patches Twisted to use permissive SSL.
    """
    ...
    def __init__(self, demo: bool = True):
        ...
        self._ctid_accounts: list = []
        
        logger.info(f"PatchedCtraderClient initialized")
    ...
    def start(self) -> bool:
        """
        Start the client with PATCHED SSL.
        Returns True if connected and authenticated.
        """
        logger.info("=" * 70)
        logger.info("STARTING PATCHED CTRADER CLIENT (macOS)")
        logger.info("Using NEW Access Token + Patched SSL!")
        logger.info("=" * 70)
        ...
        try:
            ...
            # PATCH: Create permissive SSL context
            logger.info("Patching Twisted SSL context...")
            ...
            logger.info("✓ SSL context created (permissive)")
            
            # Create client with the WORKING host
            ...
            # Wait for connection (we KNOW this works now)
            timeout = 20
            ...
            if not self._is_connected:
                logger.error("✗ Connection timeout - this should NOT happen!")
                return False
    ...
    def _get_account_list(self):
        """Step 2: Send ProtoOAGetAccountListByAccessTokenReq with NEW access token."""
        from ctrader_open_api.protobuf import Protobuf
        ...
        try:
            if hasattr(payload, 'ctidTraderAccount'):
                ...
                for acc in accounts:
                    ...
                    self._ctid_accounts.append(acc.ctidTraderAccountId)
                
                # Use our account ID or the first one
    ...
    def _authenticate_account(self, ctid_trader_account_id: int):
        """Step 3: Send ProtoOAAccountAuthReq with NEW access token."""
        from ctrader_open_api.protobuf import Protobuf
...
if __name__ == "__main__":
    print("=" * 70)
    print("🚀 AI FOREX TRADING SYSTEM v3.0 - PATCHED CTRADER 🚀")
    print("Using NEW Access Token + VERIFIED Endpoints + Patched SSL")
    print("=" * 70)
    print()
    
    client = PatchedCtraderClient(demo=True)
    ...
    if success and client.is_connected():
        ...
        print("\nPossible reasons:")
        print("1. NEW access token is invalid/expired")
