"""
WORKING cTrader Client for macOS
Uses permissive SSL context to bypass LibreSSL issues.
...
# NOTE: Access token needs to be obtained via Playground or REST API
# For now, let's use the one from config (but it might be expired)
...
class WorkingCtraderClient:
    ...
    Uses permissive SSL context to fix LibreSSL issues.
    """
    ...
    def start(self) -> bool:
        """
        Start the client with FIXED SSL handling.
        Returns True if connected and authenticated.
        """
        logger.info("=" * 70)
        logger.info("STARTING WORKING CTRADER CLIENT (macOS SSL FIX)")
        logger.info("=" * 70)
        logger.info(f"Host: {self.host}:{self.port}")
        ...
        try:
            ...
            # Create PERMISSIVE SSL context using pyOpenSSL
            ...
            logger.error("Make sure these are installed:")
            logger.error("  pip install ctrader-open-api pyOpenSSL")
            return False
...
if __name__ == "__main__":
    ...
    print("Using permissive SSL context (fixes macOS LibreSSL issue)")
    print("=" * 70)
    print()
    ...
    if success and client.is_connected():
        ...
        print(f"\nListening for real-time updates (20 seconds)...")
        time.sleep(20)
        ...
        print("\nPossible reasons:")
        print("1. Access token expired (use Playground to get fresh one)")
