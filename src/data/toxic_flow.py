...
class ToxicFlowDetector:
    ...
    def __init__(self, lookback: int = 100, bucket_size: int = 1000, toxic_threshold: float = 0.8):
    ...
    def _calculate_snapshot(self) -> ToxicFlowSnapshot:
        ...
        recent_imbalance = np.mean(self._imbalance_history[-10:]) if self._imbalance_history else 0.0
        direction = "buy" if recent_imbalance > 0.3 else "sell" if recent_imbalance < -0.3 else "neutral"

        if is_toxic:
            logger.warning(f"Toxic flow detected: VPIN={vpin:.2f}, direction={direction}")
        ...
        for tick in ticks:
            ...
            if current_vol >= self.bucket_size:
                buy_vol = sum(
                    ...
                    if t.get("price", t.get("mid", 0)) > self._get_mid(current_bucket[0])
                )
                sell_vol = sum(
                    t.get("volume", 1)
                    for t in current_bucket
    ...
    def should_fade(self, symbol: str = "") -> Tuple[bool, str]:
        ...
        recent_vpin = np.mean(self._vpin_history[-3:]) if len(self._vpin_history) >= 3 else self._vpin_history[-1]
        if recent_vpin > 0.85:
            direction = "sell" if self._imbalance_history and np.mean(self._imbalance_history[-3:]) > 0 else "buy"
