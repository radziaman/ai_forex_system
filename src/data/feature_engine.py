...
class FeatureEngine:
    ...
    def compute_features(
        ...
        for tf in self.TIMEFRAMES:
            ...
            feats.append(
                np.clip(
                    obv[-1]
                    / max(abs(np.max(obv[-20:])) if len(obv) >= 20 else 1.0, 1e-8),
                    -1,
                    1,
                )
            )
            ...
            feats += [
                np.clip(cvd_val / 1e9, -1, 1),
                np.clip((order_flow or {}).get(f"{tf}_cvd_slope", 0.0), -1, 1),
            ]
            feats += [np.clip((order_flow or {}).get(f"{tf}_imbalance", 0.0), -1, 1)]
            feats.append(
                np.clip(
                    (order_flow or {}).get(f"{tf}_large_z", 0.0) if order_flow else 0.0,
                    -3,
                    3,
                )
            )
            ...
            np.mean(
                [
                    self._atr(
                        ohlcv[t]["high"].values,
                        ohlcv[t]["low"].values,
                        ohlcv[t]["close"].values,
                        14,
                    )
                    for t in self.TIMEFRAMES
                    if t in ohlcv and len(ohlcv[t]) > 14
                ]
                or [0.0]
            )
        )
        feats.append(
            np.mean(
                [
                    self._rsi(ohlcv[t]["close"].values, 14)
                    for t in self.TIMEFRAMES
                    if t in ohlcv and len(ohlcv[t]) > 14
                ]
                or [50.0]
        ...
        feats.append(
            np.mean(
                [
                    np.log1p(ohlcv[t]["volume"].values[-1])
                    for t in self.TIMEFRAMES
                    if t in ohlcv and len(ohlcv[t]) > 0
                ]
                or [0.0]
            )
        )
        if acc:
            feats += [
                min(acc.get("balance", 0) / 100000.0, 2.0),
                min(acc.get("equity", 0) / 100000.0, 2.0),
                acc.get("margin", 0) / max(acc.get("balance", 1), 1.0),
            ]
        else:
            feats += [0.5, 0.5, 0.0]
        ...
        feats.append(
            np.clip(
                sum(p.get("unrealized_pnl", 0) for p in (positions or [])) / 1000.0,
                -1,
                1,
            )
        )
    ...
    def _atr(self, h, l, c, p=14):
        ...
        tr = np.maximum(
            h[-p:] - l[-p:],
            np.maximum(
                np.abs(h[-p:] - c[-p - 1 : -1]), np.abs(l[-p:] - c[-p - 1 : -1])
            ),
        )
        return np.mean(tr)
