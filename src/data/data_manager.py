...
class DataManager:
    ...
    def __init__(self, historical_path="data/historical", redis_url=None):
        ...
        for tf in ["1m","5m","15m","1h","4h"]:
            self.ohlcv[tf] = pd.DataFrame(columns=['timestamp','open','high','low','close','volume'])
        logger.info("DataManager initialized")

    def update_tick(self, symbol, bid, ask, volume=0, ts=None):
        ts = ts or pd.Timestamp.now().timestamp()
        mid = (bid+ask)/2.0
        self.tick_buffer.append({'ts':ts,'symbol':symbol,'bid':bid,'ask':ask,'mid':mid,'vol':volume})
    ...
    def _aggregate_1m(self, symbol, ts, price, volume):
        ...
        if df.empty or df.iloc[-1]['timestamp'] < bar_ts:
            new = pd.DataFrame([{'timestamp':bar_ts,'open':price,'high':price,'low':price,'close':price,'volume':volume}])
    ...
    def _propagate(self):
        ...
        for tf,minutes in [("5m",5),("15m",15),("1h",60),("4h",240)]:
            res=df_1m.resample(f'{minutes}T').agg({'open':'first','high':'max','low':'min','close':'last','volume':'sum'}).dropna()
            res=res.reset_index()
            res['timestamp']=res['datetime'].astype(np.int64)//10**9
            self.ohlcv[tf]=res[['timestamp','open','high','low','close','volume']]
    ...
    def _update_of(self, bid, ask, volume):
        ...
        mid=(bid+ask)/2.0
        if self._bid_hist:
            delta=volume if mid>(self._bid_hist[-1]+self._ask_hist[-1])/2.0 else -volume
        ...
        for tf in ["1m","5m","15m","1h","4h"]:
            ...
            self.order_flow[f"{tf}_cvd_slope"]=(self._cvd_hist[-1]-self._cvd_hist[-20])/20 if len(self._cvd_hist)>=20 else 0.0
            self.order_flow[f"{tf}_imbalance"]=(bid-ask)/(ask-bid) if (ask-bid)>0 else 0.0
    ...
    def get_snapshot(self, symbol="EURUSD", acc=None, positions=None):
        ...
        snapshot = type('Snapshot',(), {
            'symbol':symbol,'timestamp':pd.Timestamp.now().timestamp(),
            'features':fe.compute_features(self.ohlcv,self.order_flow,acc,positions),
            'feature_names':fe.feature_names,
            'regime':'trending_up','regime_conf':0.8,
            'cvd':self._cvd_state,'bid_ask_imbalance':self.order_flow.get('1m_imbalance',0.0)
        })()
    ...
    def _gen_synthetic(self,symbol,tf,days):
        secs=self.feature_engine.TF_MULT.get(tf,60) if hasattr(self,'feature_engine') else 60
        ...
        if "XAU" in symbol: base=2000.0
        prices=base*np.exp(np.cumsum(np.random.normal(0,0.0001,periods)))
        data=[{'timestamp':d.timestamp(),'open':p,'high':p*1.0001,'low':p*0.9999,'close':p*1.00005,'volume':int(np.random.exponential(50000))} for d,p in zip(dates,prices)]
