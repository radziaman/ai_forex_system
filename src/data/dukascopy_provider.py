...
class DukascopyDataProvider(DataProvider):
    ...
    async def fetch_ticks(self, symbol: str, date: str) -> List[PriceTick]:
        ...
        for hour in range(24):
            url = BASE_URL.format(
                symbol=duk, year=dt.year, month=dt.month, day=dt.day, hour=hour
            )
            ...
            if self.cache_enabled and cache_path.exists():
                ...
                try:
                    resp = await loop.run_in_executor(
                        None, lambda: self._scraper.get(url, timeout=15)
                    )
    ...
    async def fetch_ohlcv(
        ...
        if not all_ticks:
            return []
        tick_tuples = [
            (t.timestamp, t.bid, t.ask, t.volume, t.volume) for t in all_ticks
        ]
