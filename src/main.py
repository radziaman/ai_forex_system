...
class InstitutionalForexMoneybot:
    def __init__(self, config_path="config.yaml"):
        ...
        self.execution = ExecutionEngine(self.ctrader, self.risk_manager, self.data_manager)
        self.ai_agent = None
    ...
    async def start(self):
        ...
        threading.Thread(
            target=lambda: uvicorn.run(app, host=d_cfg['host'], port=d_cfg['port'], log_level="error"),
            daemon=True
        ).start()
    ...
    async def _trading_cycle(self):
        ...
        action, sl_mult, tp_mult, pos_size, ai_info = self.ai_agent.select_action(snapshot.features)
        ...
        if action == 1:
            await self.execution.open_position("EURUSD", "BUY", volume, sl_price, tp_price, str(ai_info))
        elif action == 2:
            await self.execution.open_position("EURUSD", "SELL", volume, sl_price, tp_price, str(ai_info))
        elif action == 3:
        ...
        if self.trade_count % self.config['ai']['finetune_freq'] == 0 and self.trade_count > 0:
    ...
    def _train_ai(self, state, action, reward):
        if self.ai_agent and self.trading_env:
            ...
            if len(self.ai_agent.states) >= 2048:
                ...
                self.ai_agent.save(f"{self.config['data']['models_path']}/ppo_latest.pt")
    ...
    def _update_dashboard(self, snapshot, acc_info, open_pos):
        update_state(
            ...
            market_data={'bid': snapshot.bid, 'ask': snapshot.ask, 'spread': snapshot.spread} if snapshot else {},
            ai_metrics={'regime': snapshot.regime if snapshot else "", 'confidence': self.last_confidence, 'action': self.last_ai_action}
        )
        try:
