...
async def main():
    ...
    for i in range(steps):
        ...
        symbols_with_data = fresh_data.get('fresh', 0) if isinstance(fresh_data, dict) else 0
        ...
        print(f"    agents: {health['alive']}/{health['total']} alive  |  "
              f"connected: exec={exec_connected} telegram={telegram_ok}")
        print(f"    bus: {bus['total_messages']} msgs ({bus['avg_latency_ms']:.1f}ms avg)  |  "
              f"dropped={bus['dropped']}  acks={bus['ack_count']}  routes={bus['routing_count']}")
        print(f"    data: {tick_rate} ticks/s  |  {symbols_with_data}/24 symbols fresh  |  "
              f"regime={regime}")
        print(f"    pipeline: {trades} trades  |  "
...
master={emotions.get('master_agent','?')}")
    ...
    print(f"\n  AGENT SUMMARY:")
    for name, reg in sorted(health.get('agents', {}).items(),
                             key=lambda x: x[0]):
        c = next((a.consciousness for a in orch.agents if a.name == name), None)
        if c:
            e = c.emotion.summary()
            print(f"  [{reg['domain']:>14}] {name:<22} "
