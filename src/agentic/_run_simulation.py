...
async def main():
    ...
    for i in range(steps):
        ...
        top_agents = sorted(
            agent_states.items(), key=lambda x: x[1]["cycles"], reverse=True
        )[:5]
        ...
        print(f"           top: {top_str}")
        print(
            f"           bus: {bus['avg_latency_ms']:.1f}ms avg, "
    ...
    for name, reg in health.get("agents", {}).items():
        ...
        print(
            f"  [{reg['domain']:>14}] {name:<22} cycles={cycles:<4} "
            f"health={reg['health']:.1f} emotion={emotion}"
        )

    print()
    print(
        f"  Bus: {bus['total_messages']} msgs, {bus['dropped']} dropped, "
        f"{bus['avg_latency_ms']:.2f}ms avg"
    )
    print(
        f"  Acks: {bus['ack_count']}, Routes: {bus['routing_count']}, "
