...
if "data.atr." in inspect.getsource(RiskAgent) and "data.atr." not in src_data:
    gaps.append(
        (
            "data.atr.{symbol}",
            "risk_agent needs ATR but data_agent never publishes",
            "HIGH",
            "Publish ATR per symbol in data_agent",
        )
    )
...
if (
    "data.primary_symbol" in src_regime
    and "data.primary_symbol"
    not in inspect.getsource(AgenticOrchestrator._init_world_state)
):
    gaps.append(
        (
            "data.primary_symbol",
            "regime_agent reads it but main doesn't set it",
            "LOW",
            "Already set in main_agentic._init_world_state",
        )
    )
