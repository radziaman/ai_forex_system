...
print(f"  Saved to models/feature_norm.npz")

# Verify dimensions
expected_ppo = 55
if n_features != expected_ppo:
    print(f"  WARNING: {n_features} features but PPO expects {expected_ppo}")
    print(f"  This is normal if feature count varies by configuration")
else:
...
def robust_load(model_path, label="model"):
    ...
    try:
        ...
        w = saved.layers[1].get_weights()
        has_weights = any(np.any(w_i != 0) for w_i in w) if w else True  # Keras rand init != 0
    ...
    try:
        ...
        instance.model = saved
        print(f"  [{label}] Attempt 2 (safe_mode=False): OK, {len(saved.layers)} layers")
        return instance
    except Exception as e:
        ...
        with zipfile.ZipFile(model_path) as z:
            ...
            if 'config.json' in names:
                ...
                try:
                    ...
                    weight_files = [n for n in names if n.endswith('.h5') or 'weights' in n]
                    if weight_files:
...
class MetaNet(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            nn.Linear(16, 128), nn.LayerNorm(128), nn.LeakyReLU(0.01), nn.Dropout(0.15),
            nn.Linear(128, 64), nn.LayerNorm(64), nn.LeakyReLU(0.01), nn.Dropout(0.15),
            nn.Linear(64, 32), nn.LayerNorm(32), nn.LeakyReLU(0.01), nn.Dropout(0.15),
            nn.Linear(32, 5),
        )
    def forward(self, x):
        return self.net(x)
...
if lstm and lstm.model:
    ...
    print(f"  LSTM: fresh (no saved weights found)")
