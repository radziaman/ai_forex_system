...
src_packages = {d.name for d in pathlib.Path('src').iterdir() if d.is_dir()
                and d.name != 'agentic' and not d.name.startswith('_')}
...
print(f"  Agentic files: {sum(1 for f in agentic_dir.rglob('*.py') if not f.name.startswith('_'))}")
