...
class InstrumentScreenerAgent(BaseAgent):
    ...
    async def perceive(self) -> Dict[str, Any]:
        ...
        while not self._inbox.empty():
            try:
                ...
                if msg.msg_type == MessageType.SCREENING_REQUEST:
                    has_request = True
                    self.log_state(f"On-demand screening requested by {msg.source_agent}")
            except:
                break
        ...
        return {
            "scan": True,
            "reason": "initial" if self._last_scan_time == 0.0
                      else "scheduled" if hours_since_last >= self.scan_interval_hours
                      else "on_demand",
        }
    ...
    async def reason(self, perception: Dict[str, Any]) -> Dict[str, Any]:
        ...
        tradeable = [
            r.to_dict() for r in results if r.edge_detected
        ]
    ...
    async def act(self, decision: Dict[str, Any]):
        ...
        self.set_world("screening.tradeable_instruments", tradeable, ttl=self.scan_interval_hours * 3600)
    ...
    async def reflect(self, outcome: Dict[str, Any]):
        ...
        self._scan_in_progress = False
        self.consciousness.current_intention = "monitoring — next scan due in {self.scan_interval_hours}h"
    ...
    def _test_strategy(self, prices: np.ndarray, lookback: int,
    ...
    def _screen_instrument(self, ticker: str, info: Dict) -> Optional[ScreenResult]:
        ...
        edge = (composite > MIN_SHARPE and mom5['pf'] >= MIN_PROFIT_FACTOR and has_enough_trades)
