...
class RiskAgent(BaseAgent):
    def __init__(self, config, initial_balance: float = 100_000.0):
        super().__init__(
            ...
            domain="risk",
            capabilities={
                "kelly_sizing", "var_cvar", "drawdown_tracking",
                "circuit_breaker", "cost_verification",
                "correlation_filtering", "dynamic_confidence",
            },
            tick_interval=1.0,
    ...
    async def perceive(self) -> Dict[str, Any]:
        ...
        halted_symbols = []
        for sym in list(getattr(cb, 'last_halt_time', {}).keys()):
        ...
        for action in decision.get("actions", []):
            if action.get("type") == "alert_halt":
                ...
                self.memory.remember(
                    ...
                    importance=0.8, emotion="warning",
                )
                await self.send(
                    MessageType.RISK_ALERT,
                    payload={"type": "circuit_breaker", "symbol": sym,
            ...
            # G9: Request human confirmation for system halt
                ...
                self.memory.remember(
                    ...
                    importance=0.9, emotion="warning",
                )
                await self.send(
                    MessageType.RISK_ALERT,
                    payload={"type": "halt_request",
    ...
    async def on_message(self, message: AgentMessage):
        if message.msg_type == MessageType.SIGNAL_GENERATED:
            ...
            await self.send(MessageType.DIAGNOSTIC_RESULT, payload={
                "agent": self.name, "approved": self._approved, "rejected": self._rejected,
                "kill_switch": self.risk_manager.kill_switch_triggered if self.risk_manager else False,
                "effective_kelly": self._effective_kelly if hasattr(self, '_effective_kelly') else 'N/A',
                "adaptive_halted": self._adaptive_halted if hasattr(self, '_adaptive_halted') else False,
    ...
    async def _evaluate_signal(self, message: AgentMessage):
        ...
        if self.risk_manager.kill_switch_triggered:
            self._reject(message, "Kill switch active"); return
        if self._adaptive_halted if hasattr(self, '_adaptive_halted') else False:
            self._reject(message, "Adaptive risk halt active"); return
        ...
        if open_pos_count >= max_positions:
            self._reject(message, f"Max positions ({max_positions}) reached"); return

        approved, reason = self.risk_manager.pre_trade_checks(balance, equity, margin, equity - balance)
        if not approved:
            self._reject(message, reason); return

        # G2: Use adaptive risk values instead of raw config
        kelly_fraction = self.get_world("adaptive_risk.kelly", self.config.trading.kelly_fraction)
        risk_per_trade = self.get_world("adaptive_risk.risk_per_trade",
                                         self.config.trading.max_risk_per_trade)
        ...
        volume = self.risk_manager.calculate_kelly_size(
            balance, price, atr, confidence, symbol=symbol,
        )
        lot_min = 0.01
        ...
        sl_mult = self.get_world("regime.params.sl_atr", self.config.trading.sl_atr_multiplier)
        tp_mult = self.get_world("regime.params.tp_atr", self.config.trading.tp_atr_multiplier)
        ...
        self._approved += 1
        self.memory.remember(event_type="trade_approved",
            description=f"{direction} {symbol} vol={volume:.2f} (kelly={kelly_fraction:.2f})",
            importance=0.7, emotion="success")

        await self.send(MessageType.RISK_APPROVED, payload={
            "signal": payload, "volume": volume, "sl_price": sl_price,
            "tp_price": tp_price, "cost_total": cost.total, "timestamp": time.time(),
    ...
    async def _reject(self, message: AgentMessage, reason: str):
        ...
        self.memory.remember(event_type="trade_rejected",
            description=f"Rejected {message.payload.get('symbol', '?')}: {reason}",
            importance=0.5, emotion="warning")
        await self.send(MessageType.RISK_REJECTED, payload={
            "signal": message.payload, "reason": reason, "timestamp": time.time(),
        })
