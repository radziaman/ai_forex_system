...
class FeatureAgent(BaseAgent):
    ...
    def __init__(self, config):
        super().__init__(
            ...
            domain="features",
            capabilities={
                "technical_indicators", "multi_timeframe_fusion",
                "z_score_normalization", "feature_caching",
                "microstructure_features", "cross_asset_features",
                "cyclical_encoding", "hurst_exponent",
            },
            tick_interval=0.5,
    ...
    async def _on_start(self):
        ...
        if loaded:
            self.log_state(f"Loaded normalization: {len(self._feature_pipeline._means)} symbol-tf pairs")
        else:
        ...
        self.memory.know("schema_version", self.FEATURE_SCHEMA_VERSION, confidence=1.0, ttl=86400 * 30)
    ...
    async def reason(self, perception: Dict[str, Any]) -> Dict[str, Any]:
        return {"should_fit": perception.get("dfs_available", False) and self._feature_pipeline is not None}
    ...
    async def act(self, decision: Dict[str, Any]):
        if decision.get("should_fit") and not self._feature_pipeline._means:
            ...
            if dfs:
                ...
                self.log_state(f"Feature pipeline fitted and saved")
                self.set_world("feature.ready", True)

    async def reflect(self, outcome: Dict[str, Any]):
        if self.consciousness.cycle_count % 200 == 0:
            n_fitted = len(self._feature_pipeline._means) if self._feature_pipeline else 0
    ...
    async def on_message(self, message: AgentMessage):
        if message.msg_type == MessageType.FEATURES_READY:
                ...
                if dfs:
                    ...
                    if features is not None:
                        await self.send(
                            ...
                            payload={"symbol": symbol, "features": features, "timestamp": time.time()},
                        )
            ...
            n_means = len(self._feature_pipeline._means) if self._feature_pipeline else 0
            await self.send(
                MessageType.DIAGNOSTIC_RESULT,
                payload={"agent": self.name, "n_features": n_cols,
                         "n_fitted": n_means, "schema": self.FEATURE_SCHEMA_VERSION},
