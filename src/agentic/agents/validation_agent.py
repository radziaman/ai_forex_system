...
class ValidationAgent(BaseAgent):
    def __init__(self):
        super().__init__(
            ...
            domain="validation",
            capabilities={
                "vectorized_backtesting", "walk_forward_optimization",
                "monte_carlo_testing", "stress_testing",
                "degradation_tracking", "sensitivity_analysis",
                "ab_testing",  # G20
    ...
    async def reason(self, perception: Dict[str, Any]) -> Dict[str, Any]:
        should_run = (time.time() - self._last_validation_time > 3600
                      and perception.get("trades_available"))
    ...
    async def on_message(self, message: AgentMessage):
        if message.msg_type == MessageType.TRAINING_REQUEST:
            ...
            await self.send(MessageType.VALIDATION_RESULT, payload=result,
                target=message.source_agent, priority=MessagePriority.NORMAL,
            ...
            await self.send(MessageType.DIAGNOSTIC_RESULT, payload={
                "agent": self.name, "last_validation": self._last_validation_time,
                "ab_experiments": len(self._ab_experiments),
                "cached_data": {"prices": len(self._cached_prices),
    ...
    async def _fetch_data(self):
        ...
        # Try to get DataManager from data_agent
        data_agent_msg = AgentMessage(
            msg_type=MessageType.DIAGNOSTIC_REQUEST,
            source_agent=self.name,
            target_agent="data_agent",
            payload={"request": "ohlcv_snapshot"},
        )
        self._cached_trades = self.get_world("performance.stats", {}).get("total_trades", 0)
        self.log_state(f"Data fetched for validation: {len(self._cached_prices)} price series")

    # G20: Start an A/B experiment
    async def start_ab_test(self, test_id: str, model_a: Callable,
                             model_b: Callable, data: Dict) -> str:
        experiment = {
            "id": test_id, "model_a": model_a, "model_b": model_b,
            "data": data, "started_at": time.time(),
            "results_a": [], "results_b": [],
        }
    ...
    async def _run_validation(self, validation_type: str, params: Dict) -> Dict:
        ...
        result = {"type": validation_type, "timestamp": time.time(),
                  "passed": False, "summary": {}, "details": {}}
        ...
        if validation_type in ("full", "monte_carlo"):
            ...
            if mc_result:
                result["summary"]["mc_significant"] = mc_result.get("is_significant", False)

        result["passed"] = all(
            result.get("details", {}).get(k, {}).get("passed", False)
            for k in result["details"]
        ) if result["details"] else False
        ...
        if not result["passed"]:
            self.memory.remember(event_type="validation_failed",
                description=f"{validation_type} failed", importance=0.8, emotion="warning",
                data=result["summary"])
        else:
            self.memory.remember(event_type="validation_passed",
                description=f"{validation_type} passed", importance=0.6, emotion="success")
    ...
    async def _run_backtest(self, params: Dict) -> Dict:
        try:
            ...
            result = bt.run(prices=prices, signal_fn=signal_fn,
                            features=params.get("features"), atr=params.get("atr"),
                            regimes=params.get("regimes"))
            return {"passed": result.sharpe > 0.5, "sharpe": result.sharpe,
                    "win_rate": result.win_rate, "profit_factor": result.profit_factor,
                    "max_dd": result.max_drawdown_pct, "total_trades": result.total_trades,
    ...
    async def _run_walk_forward(self, params: Dict) -> Dict:
        try:
            ...
            wf = SmartWalkForward(n_folds=6, test_window=252, embargo=10,
                                   use_cpcv=True, n_combinations=10)
            prices = params.get("prices", np.array([]))
            if len(prices) < 600:
                return {"passed": False, "error": "insufficient_data"}
            result = wf.run(prices=prices, features_fn=params.get("features_fn", lambda *a: []),
                            features=params.get("features"), regimes=params.get("regimes"))
            return {"passed": result.passed, "avg_train_sharpe": result.avg_train_sharpe,
                    "avg_test_sharpe": result.avg_test_sharpe,
                    "avg_degradation": result.avg_degradation,
                    "stability_score": result.stability_score, "n_folds": result.total_folds}
        except Exception as e:
            return {"passed": False, "error": str(e)}
    ...
    async def _run_monte_carlo(self, params: Dict) -> Dict:
        try:
            ...
            return {"passed": result.is_significant_sharpe,
