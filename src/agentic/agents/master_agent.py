...
class MasterAgent(BaseAgent):
    def __init__(self):
        super().__init__(
            ...
            capabilities={
                "system_health_monitoring", "agent_coordination",
                "anomaly_detection", "self_healing", "directive_issuance",
                "global_state_management", "error_escalation",  # G22
    ...
    async def perceive(self) -> Dict[str, Any]:
        ...
        dead_agents = {k: v for k, v in report.get("agents", {}).items()
        ...
        return {"total_agents": total, "alive": alive, "health_pct": health_pct,
                "dead_agents": dead_agents, "domain_health": domain_health,
    ...
    async def act(self, decision: Dict[str, Any]):
        for directive in decision.get("directives", []):
            ...
            await self.send(MessageType.AGENT_DIRECTIVE,
                payload={"action": directive["action"], "reason": directive["reason"]},
                target=directive["target"], priority=MessagePriority.HIGH,
    ...
    async def reflect(self, outcome: Dict[str, Any]):
        ...
        self.set_world("master.domain_health", outcome.get("reports", {}).get("domain_health", {}))
        ...
        if message.msg_type == MessageType.AGENT_ERROR:
            ...
            if payload.get("requires_confirmation"):
                ...
                self._pending_approvals[approval_id] = {
                    ...
                    "approved": False, "timeout": 300,
                }
                self.log_state(f"HALT requires human approval (id={approval_id}) — waiting 5min", "critical")
                self.memory.remember(event_type="human_approval_pending",
                    description=f"Halt pending human approval #{approval_id}",
                    importance=1.0, emotion="warning")
                return
            ...
            if alert_type == "halt":
                await self.send(MessageType.AGENT_DIRECTIVE,
                    ...
                    target="execution_agent", priority=MessagePriority.CRITICAL)

        elif message.msg_type == MessageType.DIAGNOSTIC_REQUEST:
            registry = get_agent_registry()
            await self.send(MessageType.DIAGNOSTIC_RESULT, payload={
                "agent": self.name, "system_health": self._system_health,
    ...
    async def _on_agent_error(self, message: AgentMessage):
        ...
        self.log_state(f"Error from {source}: {consecutive} consecutive failures", "warning")

        # Escalate if critical
        if consecutive >= 5:
            self.log_state(f"CRITICAL: {source} has {consecutive} consecutive errors", "critical")
            await self.send(MessageType.RISK_ALERT,
                payload={"type": "agent_critical", "agent": source,
        ...
        if agent_reg and agent_reg.supervisor:
            await self.send(MessageType.AGENT_DIRECTIVE,
                payload={"action": "investigate_agent", "agent": source,
    ...
    async def _run_system_diagnostics(self):
        ...
        dead = [k for k, v in report.get("agents", {}).items()
                if not v.get("alive", True) and k != self.name]
        degraded = [k for k, v in report.get("agents", {}).items()
                    if v.get("health", 1) < 0.5 and k != self.name]
        error_sources = list(set(e["source"] for e in self._error_log[-50:]))

        if dead:
            self.memory.remember(event_type="diagnostic_dead_agents",
                description=f"Dead: {dead}", importance=0.8, emotion="warning")
        if degraded:
            self.memory.remember(event_type="diagnostic_degraded",
                description=f"Degraded: {degraded}", importance=0.6, emotion="warning")

        self.set_world("master.last_diagnostics", {
            "time": time.time(), "dead": dead, "degraded": degraded,
            "error_sources": error_sources, "health": self._system_health,
            "pending_approvals": len(self._pending_approvals),
        })
