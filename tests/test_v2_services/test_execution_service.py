...
class TestExecutionService:
    ...
    def test_execute_returns_result(self, mock_engine_cls, mock_provider):
        ...
        mock_engine.open_position = AsyncMock(return_value=MagicMock(
            entry_price=1.1205, position_id=42,
        ))
        ...
        svc = ExecutionService(self.config, MagicMock(), mock_pipeline)
        ...
        decision = TradeDecision(signal=signal, volume=1000, sl_price=1.11, tp_price=1.14)
    ...
    def test_execute_returns_failure_on_order_error(self, mock_provider):
        ...
        with patch('services.execution_service.ExecutionEngine', return_value=mock_engine):
            svc = ExecutionService(self.config, MagicMock(), mock_pipeline)
            asyncio.run(svc.start())
