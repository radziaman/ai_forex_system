...
try:
    ...
    check("regime_detector", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("ensemble", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("model (AI)", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("drift_detector", lambda: (_ for _ in ()).throw(e))  # noqa: F821

try:
    from rts_ai_fx.attention_fusion import TemporalAttentionFusion

    check("attention_fusion", lambda: True)
except Exception as e:
    check("attention_fusion", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("uncertainty", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("infrastructure.config_v2", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("infrastructure.secrets", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("risk.manager", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("risk.circuit_breaker", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("execution.engine", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("execution.cost_model", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("execution.algo_executor", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("data.data_manager", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("data.market_session", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("data.economic_calendar", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("ai.sentiment", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("ai.regime_agents", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("ai.maml_agent", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("training.model_registry", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("training.online_learner", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("validation.walk_forward", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("validation.monte_carlo", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check(
        "backtest.vectorized_backtester", lambda: (_ for _ in ()).throw(e)  # noqa: F821
    )
...
try:
    ...
    check("notifications.telegram", lambda: (_ for _ in ()).throw(e))  # noqa: F821
...
try:
    ...
    check("dashboard.app", lambda: (_ for _ in ()).throw(e))  # noqa: F821
